...
from .models import (
    Task, Worker, Project, Tag, Team, ActivityLog,
    Notification, Position
)
from .forms import (
    TaskForm, WorkerRegistrationForm, ProjectForm,
    TeamForm, TagForm, CommentForm
)
...
class TaskUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if task.created_by != request.user and not request.user.is_staff:
            messages.error(
                request,
                'У вас немає прав для редагування цього завдання'
            )
...
def task_toggle_status(request, pk):
    ...
    if task.is_completed:
        ...
        messages.info(
            request, f'Завдання "{task.name}" позначено як невиконане'
        )
    else:
        task.mark_as_completed()
        messages.success(
            request, f'Завдання "{task.name}" позначено як виконане!'
        )
...
class ProjectDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['tasks'] = self.object.tasks.all().select_related(
...
class TeamDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['members'] = self.object.members.all().select_related(
            'position'
        )
        return context
...
def project_add_team(request, pk):
    ...
    if request.method == 'POST':
        ...
        if team_id:
            ...
            messages.success(
                request,
                f'Команду "{team.name}" успішно додано до проекту!'
            )
...
def team_add_member(request, pk):
    ...
    if request.method == 'POST':
        ...
        if member_id:
            ...
            messages.success(
                request,
                f'Користувача "{member.get_full_name()}" додано до команди!'
            )
...
def team_remove_member(request, pk, member_pk):
    ...
    if request.method == 'POST':
        ...
        messages.success(
            request,
            f'Користувача "{member.get_full_name()}" видалено з команди!'
        )
        return redirect('team_detail', pk=team.pk)
...
def dashboard(request):
    ...
    priority_stats_raw = Task.objects.filter(
        is_completed=False
    ...
    # Остання активність
    recent_activity = ActivityLog.objects.all().select_related(
        'task', 'user'
...
class NotificationListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = self.request.user.notifications.all().select_related(
            'task'
        ).order_by('-created_at')
...
class PositionCreateView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request,
            f'Посаду "{form.instance.name}" успішно створено!'
        )
        return super().form_valid(form)
...
class PositionUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request,
            f'Посаду "{form.instance.name}" успішно оновлено!'
        )
        return super().form_valid(form)
...
class PositionDeleteView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(
            request, f'Посаду "{position.name}" успішно видалено!'
        )
        return super().delete(request, *args, **kwargs)
...
class WorkerUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request,
            f'Дані працівника "{form.instance.get_full_name()}" оновлено!'
        )
        return super().form_valid(form)
